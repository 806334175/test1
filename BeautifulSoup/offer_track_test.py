...
def test_url(track, country, appid, d_type, row):
    for cols in range(2):
        threading.Thread(target=offer_test, args=(track, country, appid, d_type, row, cols,)).start()
...
if __name__ == '__main__':
    ...
    for row in range(len(list_appid) - 1):
        row += 1
        threading.Thread(target=test_url,
                         args=(list_track[row], list_county[row], list_appid[row], list_d_type[row], row,)).start()
